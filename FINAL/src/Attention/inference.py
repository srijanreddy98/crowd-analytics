...
def rotate_check(face_size):
	...
	rotate_check.size_distribution[int(face_size/100)] += 1
	
	percentage = rotate_check.size_distribution[int(face_size/100)] * 100 / rotate_check.full_count
	if percentage  < 10:
		return False
	rotate_check.size_distribution[int(face_size/100)] += 1
	return True
...
while True:
	...
	cv2.rectangle(frame, (a,b), (a+w,b+h), (255,0,255), 1)
	eyes = []
	if glasses == False:
	...
	drowsiness_check_idx += 1
            ...
            # imaegs will be shown with blue boxing.
	if rotate_check(face_size) == True and continuous_error_count < 5 and drowsiness_check_list == [1]*WINDOW_SIZE:
				print("Non Attentive")
				cv2.rectangle(roi_color, (int(ex),int(ey)), (int(ex+ew), int(ey+eh)), (255,0,0), 1)
				font = cv2.FONT_HERSHEY_SIMPLEX
					#cv2.putText(roi_color,'Non-attentive',(0,130), font, 1, (200,255,155))
				cv2.putText(roi_color,'Non-attentive' , (a+w,b+h),cv2.FONT_HERSHEY_SIMPLEX,5, cv2.LINE_AA)
				eye_full_cnt += 1
				p,q,r = frame.shape
	...
	elif rotate_check(face_size) == True and not(drowsiness_check_list == [1]*WINDOW_SIZE):
				print("Attentive")
